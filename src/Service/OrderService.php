...
use Etbag\TrxpsPayments\Validator\OrderLineItemValidator;
use Etbag\TrxpsPayments\Api\Types\OrderLineType;
...
class OrderService
{
    ...
    private const TRXPS_PRICE_PRECISION = 2;
    ...
    public function getOrderLinesArray(OrderEntity $order): array
    {
        ...
        foreach ($lineItems as $item) {
            ...
            try {
                $trxpsPreparedApiPrices = $this->calculateLineItemPriceData($item, $order->getTaxStatus(), $currencyCode);
            } catch (MissingPriceLineItemException $e) {
                $this->logger->critical(
                    sprintf(
                        'The order could not be prepared for trxps api. The LineItem with id (%s), sku (%s) has no prices',
                        $item->getId(),
                        (string)$sku
                    )
                );
            ...
            $lines[] = [
                ...
                'unitPrice' => $trxpsPreparedApiPrices['unitPrice'],
                'totalAmount' => $trxpsPreparedApiPrices['totalAmount'],
                'vatRate' => $trxpsPreparedApiPrices['vatRate'],
                'vatAmount' => $trxpsPreparedApiPrices['vatAmount'],
                'sku' => $sku,
...
//            'productUrl' => null,
    ...
    /**
     * returns an array of totalPrice, unitPrice and vatAmount that is calculated like trxps api does
    ...
    public function calculateLineItemPriceData(OrderLineItemEntity $item, string $orderTaxType, string $currencyCode): array
    {
        ...
        // is a net price.
        // For correct trxps api tax calculations we have to calculate the shopware gross
        // price
        ...
        $unitPrice = round($unitPrice, self::TRXPS_PRICE_PRECISION);
        ...
        $roundedLineItemTotalPrice = round($lineItemTotalPrice, self::TRXPS_PRICE_PRECISION);
        $roundedVatRate = round($vatRate, self::TRXPS_PRICE_PRECISION);
        $vatAmount = $roundedLineItemTotalPrice * ($roundedVatRate / (100 + $roundedVatRate));
        $roundedVatAmount = round($vatAmount, self::TRXPS_PRICE_PRECISION);
        ...
        return [
            ...
            'vatRate' => number_format($roundedVatRate, self::TRXPS_PRICE_PRECISION, '.', '')
        ];
    }
    ...
    public function getPriceArray(string $currency, ?float $price = null): array
    {
        ...
        return [
            'currency' => $currency,
            'value' => number_format(round($price, self::TRXPS_PRICE_PRECISION), self::TRXPS_PRICE_PRECISION, '.', '')
        ];
    }
