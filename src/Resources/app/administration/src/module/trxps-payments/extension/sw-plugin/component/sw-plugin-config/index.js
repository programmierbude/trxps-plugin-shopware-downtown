...
Component.override('sw-plugin-config', {
    ...
    methods: {
        onTestButtonClicked() {
            ...
            const liveApiKeyInput = document.querySelector('input[name="TrxpsPayments.config.liveApiKey"]');
            const testApiKeyInput = document.querySelector('input[name="TrxpsPayments.config.testApiKey"]');

            const liveApiKey = !!liveApiKeyInput ? liveApiKeyInput.value : null;
            const testApiKey = !!testApiKeyInput ? testApiKeyInput.value : null;

            this.TrxpsPaymentsConfigService.testApiKeys({liveApiKey, testApiKey})
                .then((response) => {
